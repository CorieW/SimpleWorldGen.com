...
export default function ScrollContainer(props: Props) {
    ...
    useEffect(() => {
        if (!scrollRef.current || !scrollBarRef.current || !scrollContainerRef.current || !contentContainerRef.current) return
        ...
        // Set up scroll bar initial position and size
        updateScrollBar()
        ...
        // Update scroll bar size and position on resize of content container
        new ResizeObserver(() => {
            updateScrollBar()
        }).observe(contentContainer)
        ...
        function updateScrollBar() {
            ...
            if (scrollContainerWidth === scrollContainerScrollWidth && mode === 'horizontal') {
                scroll.style.display = 'none'
            } else if (scrollContainerHeight === scrollContainerScrollHeight && mode === 'vertical') {
                scroll.style.display = 'none'
            } else {
                scroll.style.display = 'block'
            }
    ...
    return (
        <div className={`${mode}-scroll-container`} ref={scrollContainerRef}>
            <div className='content-container' ref={contentContainerRef}>{children}</div>
            <div className={`${mode}-scroll`} ref={scrollRef}>
                <button className={`${mode}-scroll-bar`} ref={scrollBarRef}></button>
            </div>
        </div>
    )
}
